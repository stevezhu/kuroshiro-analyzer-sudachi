...
/**
 * Subset of kuroshiro's token contract this analyzer emits. kuroshiro's own
 * consumption (core.js / util.js) only reads `surface_form`, `pos`, `reading`,
 * and `pronunciation` — the wider JSDoc in `kuroshiro-analyzer-kuromoji`
 * documents fields kuroshiro never references (`pos_detail_*`, `conjugated_*`,
 * `basic_form`, `verbose`), so we omit them. `pronunciation` is unset because
 * Sudachi has no separate pronunciation field; kuroshiro's `pronunciation ||
 * reading` fallback (core.js:103, 168) lands on the right value either way.
 */
...
export interface SudachiAnalyzerOptions {
   ...
   * sudachi-wasm333 falls back to the `system.dic` bundled in its `resources/`
   * directory. Provide this to use a larger SudachiDict (small / core / full)
   * from https://github.com/WorksApplications/SudachiDict or
   * http://sudachi.s3-website-ap-northeast-1.amazonaws.com/sudachidict/
   */
  dictPath?: string;
  /**
   * Sudachi split mode. Defaults to `TokenizeMode.C` (longest tokens — keeps
   * compounds and named entities intact, the right choice for furigana / romaji
   * output).
   */
  mode?: TokenizeMode;
}
...
/**
 * Kuroshiro analyzer backed by Sudachi (via sudachi-wasm333). Conforms to the
 * analyzer interface kuroshiro.init() accepts: an object with async `init()`
 * and `parse(text)` methods.
 *
 * @example
...
export default class SudachiAnalyzer {
  ...
  /**
   * Loads the SudachiDict file and prepares the tokenizer. Must be called once
   * before `parse`. Rejects if already initialized.
   */
   ...
   * Whitespace inside non-empty input is preserved as Sudachi 空白-POS tokens.
   */
